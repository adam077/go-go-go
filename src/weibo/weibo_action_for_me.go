...
func EditWeiBo(cookie, mid, text string, picIds []string) (string, error) {
	/*
		发一个微博
	*/
	headers := map[string]string{
	...
	result, err := utils.QueryPostWithFormData("https://weibo.com/p/aj/v6/mblog/edit", nil, headers, form)
	if err != nil {
		log.Error().Msg(err.Error())
		return "", err
	}
